...
(function () {
  ...
  function selectModel(modeId, tag, done) {
    ...
    // Menu renders async; look for a menuitem whose text matches the wanted mode.
    setTimeout(function () {
      var items = [].slice.call(document.querySelectorAll('[role="menuitem"], [role="option"], button, a'));
      var hit = null;
      for (var i = 0; i < items.length; i++) {
        if (!isVisible(items[i])) continue;
        var t = (items[i].textContent || '').trim().toLowerCase();
        if (t && t.indexOf(want) === 0) { hit = items[i]; break; }   // "fast", "expert", …
      }
      if (hit) {
        ...
        wlog('job ' + tag + ' model "' + want + '" not in menu (free tier? closing) — using current');
  ...
  function pollLoop() {
    ...
    if (!HIVE_KEY) { _polling = false; return; }   // not registered yet — bootstrap will start us
  ...
  function ensureKeyThen(start) {
    ...
    proxyPost('/register', { label: '' }, function (j, status) {
      if (j && j.key) {
        ...
        // the shared refresh-token worker so the script still works.
        if (status === 403 || status === 404) { console.log('[Grok2API] no hive on server — legacy relay mode'); start(); return; }
  ...
  // ── Noob-friendly key panel: shows THIS user's API key with a copy button. ──
  var kEl = null;
  function showKeyPanel() {
    try {
      ...
      if (!kEl) {
        ...
        kEl.style.cssText = BADGE_BASE + ';top:34%;border-color:#2fbf71;max-width:330px;text-align:center';
        (document.body || document.documentElement).appendChild(kEl);
        makeDraggable(kEl);
      }
      kEl.innerHTML = '';
      var title = document.createElement('div');
      title.textContent = '⠿ Your Grok2API key — use as the API key';
      title.style.cssText = 'margin-bottom:5px;font-weight:600';
      var code = document.createElement('div');
      code.textContent = HIVE_KEY;
      code.style.cssText = 'font:11px/1.3 ui-monospace,monospace;word-break:break-all;background:#0c0e12;padding:5px 7px;border-radius:6px;margin-bottom:6px';
      var btn = document.createElement('button');
      btn.textContent = 'Copy key';
      btn.style.cssText = 'cursor:pointer;border:0;border-radius:6px;padding:5px 12px;background:#2fbf71;color:#06210f;font-weight:700;font-size:12px';
      btn.addEventListener('click', function (e) {
        e.stopPropagation();
        var ok = function () { btn.textContent = 'Copied ✓'; setTimeout(function () { btn.textContent = 'Copy key'; }, 1500); };
        try { (W.navigator.clipboard && W.navigator.clipboard.writeText(HIVE_KEY).then(ok).catch(fallback)) || fallback(); }
        catch (x) { fallback(); }
        function fallback() {
          try { var ta = document.createElement('textarea'); ta.value = HIVE_KEY; document.body.appendChild(ta); ta.select(); document.execCommand('copy'); ta.remove(); ok(); }
          catch (y) { btn.textContent = 'select & copy above'; }
        }
      });
      kEl.appendChild(title); kEl.appendChild(code); kEl.appendChild(btn);
    } catch (e) {}
  }
  ...
  var bi = setInterval(function () {
    if (document.body) {
      clearInterval(bi);
      badge(HIVE_KEY ? 'starting…' : 'first run — registering this tab…', true);
      if (HIVE_KEY) showKeyPanel();
    }
  }, 300);
