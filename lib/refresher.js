...
(function () {
  ...
  function findSendButton(input) {
    ...
    if (form) cands = cands.concat([].slice.call(form.querySelectorAll('button')));
    cands = cands.concat([].slice.call(document.querySelectorAll('button[type="submit"], button[aria-label*="Send" i], button[aria-label*="Submit" i], button[data-testid*="send" i]')));
  ...
  function sendNow(input, tag) {
    var btn = findSendButton(input);
    if (btn) { wlog('job ' + tag + ' clicking send button'); try { btn.click(); return; } catch (e) {} }
    wlog('job ' + tag + ' dispatching Enter on input');
    var types = ['keydown', 'keypress', 'keyup'];
    for (var i = 0; i < types.length; i++) {
      try { input.dispatchEvent(new KeyboardEvent(types[i], { key: 'Enter', code: 'Enter', keyCode: 13, which: 13, bubbles: true, cancelable: true })); } catch (e) {}
    }
  }
  ...
  function afterNewChat(job, tag) {
    ...
    setTimeout(function () {
      if (pendingCapture && pendingCapture.job === job) {
        pendingCapture = null;
        endJob(job, 'grok issued no chat request after send — send control not triggered (UI changed?)');
      }
    }, 15000);
    // Give React a tick to enable the send button, then send.
    setTimeout(function () { sendNow(input, tag); }, 150);
  }
