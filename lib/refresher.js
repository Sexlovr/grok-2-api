...
(function () {
  ...
  function runJob(job) {
    // We must sign the request ourselves. grok attaches x-statsig-id INSIDE its
    // own code before calling fetch, so a fetch WE initiate is NOT auto-signed
    // (that's the "403 from the phone" symptom). But we passively skim grok's
    // sigs off its real traffic into lastSig, and a sig is reusable for a few
    // minutes — so attach the freshest skimmed sig explicitly.
    if (!lastSig) {
      proxyPost('/relay/finish', { id: job.id, error: 'phone worker has no x-statsig-id yet — interact with grok once (send a message) so a sig can be skimmed' });
      workerBadge('job ' + job.id.slice(-6) + ': no sig yet — send a msg in grok once', false);
      // Nudge grok into making a signed request so we capture a sig for next time.
      keepalive();
      return;
    }
    workerBadge('job ' + job.id.slice(-6) + ' → calling grok…', true);
    var batch = [];
    var flushTimer = null;
    function flush() {
    ...
    // Native fetch from the phone tab: residential IP + the tab's cookies +
    // cf_clearance, AND the skimmed x-statsig-id we attach by hand. credentials
    // 'include' sends the grok session/clearance cookies automatically.
    W.fetch('/rest/app-chat/conversations/new', {
      method: 'POST', credentials: 'include',
      headers: {
        'accept': '*/*',
        'content-type': 'application/json',
        'x-statsig-id': lastSig,
      },
      body: buildGrokBody(job),
    }).then(function (resp) {
      // Tell the proxy the upstream status (lets it distinguish a 403 etc.).
      ...
      if (!resp.ok) {
        return resp.text().then(function (t) {
          ...
          workerBadge('job ' + job.id.slice(-6) + ' grok HTTP ' + resp.status, false);
          // 403 likely means the skimmed sig aged out mid-flight. Drop it and
          // nudge grok for a fresh one so the next job has a valid sig.
          if (resp.status === 403) { lastSig = null; keepalive(); }
        });
      }
      // Stream the NDJSON body line-by-line back to the proxy.
      ...
      function pump() {
        return reader.read().then(function (r) {
          if (r.done) {
            ...
            workerBadge('job ' + job.id.slice(-6) + ' done ✓', true);
            return;
          }
      ...
      workerBadge('job ' + job.id.slice(-6) + ' failed: ' + (e && e.message), false);
    });
  }
  ...
  function pollLoop() {
    ...
    proxyPost('/relay/poll', {}, function (j, status) {
      if (status === 401) { workerBadge('worker: bad refresh token — reinstall script from the proxy', false); setTimeout(pollLoop, 15000); return; }
      if (j && j.job) { try { runJob(j.job); } catch (e) {} setTimeout(pollLoop, 50); return; }
