...
(function () {
  ...
  function teeResponse(resp, job) {
    ...
    if (!resp.ok && (resp.status === 429 || resp.status >= 500)) {
      ...
      if (n < 3) {
        ...
        setTimeout(function () { afterNewChat(job, tag); }, wait);   // busy stays true across the retry
        return;
      }
  ...
  function afterNewChat(job, tag) {
    ...
    // 2.5s between attempts — long enough for a successful submit's request to
    // fire and clear pendingCapture BEFORE the next attempt, so we don't
    // double-submit (which would waste a rate-limit slot and clutter history).
    var sendTries = 0;
    (function loop() {
      if (!pendingCapture || pendingCapture.job !== job) return;  // fired/captured → stop
      sendTries++;
