...
<div class="wrap">
  ...
  <div id="app" class="hide">
    ...
    <div class="card">
      ...
      <p class="small mut">A headed Chromium runs inside the Space and feeds fresh sigs automatically — no phone tab needed. Two ways to use it: <b>(1) No account yet?</b> Just click <b>Launch browser</b> — it opens grok.com in <b>login mode</b>; log in once via the recovery console below and the account (sso cookies + sig) is <b>captured automatically</b>. <b>(2) Already added an account?</b> Launch reopens it already logged in. It does <b>not</b> start on its own.</p>
...
// localStorage access instead of returning null, which would kill the whole
// inline script and leave the page stuck on "loading". Guard every access.
...
async function login(){try{const j=await api('/admin/login',{method:'POST',body:JSON.stringify({password:$('#pw').value})});TOK=j.token;SS.set('g_tok',TOK);show();}catch(e){toast('Login failed',1);}}
