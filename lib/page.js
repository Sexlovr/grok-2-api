...
async function loadStatus(){try{const s=await api('/admin/status');$('#rtHint').textContent=s.refreshTokenHint||'—';$('#status').innerHTML='account: <b class="'+(s.hasAccount?'ok':'err')+'">'+(s.hasAccount?'loaded':'none')+'</b> · sig: '+sigFmt(s.sig);}catch(e){if((''+e).includes('401')){TOK='';localStorage.removeItem('g_tok');location.reload();}}}
