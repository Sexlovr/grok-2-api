...
<div class="wrap">
  ...
  <div id="app" class="hide">
    ...
    <div class="card">
      <h2>Sig refresher + phone-egress worker <span class="pill">required</span></h2>
      <p class="small mut">This userscript does two jobs on your real grok.com tab: (1) feeds a fresh <span class="code">x-statsig-id</span>, and (2) acts as the <b>egress worker</b> — it runs the actual grok chat call from your phone (residential IP + live <span class="code">cf_clearance</span>), so requests survive grok's IP-bound Cloudflare block. Install <b>Tampermonkey</b> (desktop) or <b>Violentmonkey</b> (Android/Firefox), click install, and keep a grok.com tab open & awake.</p>
...
function wkFmt(w){if(!w)return '<b class="mut">unknown</b>';if(w.online)return '<b class="ok">online</b> (seen '+w.lastSeenSeconds+'s ago)';return '<b class="err">offline</b>'+(w.lastSeenSeconds!=null?' (last seen '+w.lastSeenSeconds+'s ago)':' (never connected)');}
async function loadStatus(){try{const s=await api('/admin/status');$('#rtHint').textContent=s.refreshTokenHint||'—';$('#status').innerHTML='phone worker: '+wkFmt(s.worker)+' · account: <b class="'+(s.hasAccount?'ok':'err')+'">'+(s.hasAccount?'loaded':'none')+'</b> · sig: '+sigFmt(s.sig)+'<br><span class="small mut">phone worker online → chat egresses from your phone (survives grok\\'s IP-bound clearance). Offline → falls back to direct datacenter path (needs a fresh sig; usually 403s).</span>';}catch(e){if((''+e).includes('401')){logout();}}}
