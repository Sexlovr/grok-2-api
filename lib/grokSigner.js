...
export async function discoverSignerSource(opts = {}) {
    ...
    for (const form of forms) {
        const marker = form + ',W=>{';
        const idx = sChunk.indexOf(marker);
        if (idx >= 0) { modSrc = braceMatchArrow(sChunk, idx + form.length + 1); break; }
    }
    if (!modSrc) throw new Error(`could not extract signer module ${N2} from ${chunkFile}`);
    if (!modSrc.includes('W.s(["default"')) throw new Error('extracted module has no default export — discovery heuristic stale');
