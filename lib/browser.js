...
export class BrowserManager {
  ...
  async start({ cookieString, userAgent, proxyUrl, refreshToken }) {
    ...
    // 1) Xvfb
    this.xvfb = spawn('Xvfb', [DISPLAY, '-screen', '0', SCREEN, '-nolisten', 'tcp', '-ac'], {
      stdio: 'ignore',
    });
  ...
  screenshot() {
    return new Promise((resolve, reject) => {
      ...
      execFile('scrot', ['-o', '/tmp/grok_screen.png'], { env: { ...process.env, DISPLAY } }, (err) => {
        ...
        execFile('ffmpeg', ['-y', '-f', 'x11grab', '-video_size', '1280x800', '-i', DISPLAY, '-frames:v', '1', '/tmp/grok_screen.jpg'],
