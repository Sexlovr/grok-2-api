...
export class GrokBrowser {
    constructor(opts = {}) {
        ...
        this._seq = 0;
        this._starting = null;
        this._verifyToken = null;   // rotating grok-site-verification meta token
    }
    ...
    async loadAccount(account) {
        ...
        // Capture the rotating `grok-site-verification` meta token AS EARLY as
        // possible. grok.com serves it fresh in the SSR HTML, and the signer
        // reads it via querySelectorAll("[name^=gr]")[0].getAttribute("content")
        // and folds it into x-statsig-id. Next.js hydration can strip/replace
        // the <head>, so by signer-time the element may be gone — which crashed
        // the signer with "undefined.childNodes". We snapshot it at the earliest
        // document state, then re-assert it into the DOM before signing.
        await this.page.addInitScript(() => {
            try {
                const grab = () => {
                    const el = document.querySelector('meta[name^="grok"]');
                    if (el) window.__grokVerify = el.getAttribute('content');
                };
                grab();
        ...
        // Read the verification token straight from the freshly-served HTML
        // (before hydration can remove it). Prefer the init-script snapshot.
        const verifyToken = await this.page.evaluate(() => {
            const fromSnap = window.__grokVerify;
            const el = document.querySelector('meta[name^="grok"]');
            return fromSnap || (el && el.getAttribute('content')) || null;
        });

        // The signer reads TWO things from the page and folds both into the
        // x-statsig-id: (1) the grok-site-verification meta token (SSR <head>,
        // stripped during hydration — captured above), and (2) an animated SVG
        // "canary" element with class .r-2tw8y0 that React renders only AFTER
        // hydration (querySelectorAll('.r-2tw8y0')[0].childNodes[0].childNodes[1]
        // .getAttribute('d')). So we MUST wait for hydration for the SVG, and
        // ALSO re-assert the meta token (below) that hydration removed.
        let svgReady = false;
        try {
            await this.page.waitForSelector('.r-2tw8y0', { timeout: 30000, state: 'attached' });
            svgReady = true;
        } catch { /* logged below; injectSigner will surface the real error */ }
        await this.page.waitForTimeout(1500);

        // Diagnostics: what did this IP land on, token captured, SVG rendered?
        try {
            const diag = await this.page.evaluate(() => {
                const svg = document.querySelector('.r-2tw8y0');
                return {
                    url: location.href,
                    title: document.title,
                    metaNow: !!document.querySelector('meta[name^="grok"]'),
                    snap: !!window.__grokVerify,
                    svgCount: document.querySelectorAll('.r-2tw8y0').length,
                    svgChildDepth: svg ? `${svg.childNodes.length}/${svg.childNodes[0]?.childNodes?.length ?? 'x'}` : 'none',
            ...
            console.log('[Grok][diag] page after load:', JSON.stringify(diag), 'tokenCaptured:', !!verifyToken, 'svgWaited:', svgReady);
        } catch (e) {
            console.log('[Grok][diag] could not read page:', e.message);
        }

        // Guarantee the meta element exists in the live DOM so the signer's
        // querySelectorAll("[name^=gr]")[0] always resolves, even post-hydration.
        if (verifyToken) {
            await this.page.evaluate((tok) => {
                let el = document.querySelector('meta[name^="grok"]');
                if (!el) {
                    el = document.createElement('meta');
                    el.setAttribute('name', 'grok-site-verification');
                    (document.head || document.documentElement).appendChild(el);
                }
                el.setAttribute('content', tok);
            }, verifyToken);
        }
    ...
    async chat({ prompt, modeId = 'fast', disableSearch = true, onToken, onThink }) {
        ...
        try {
            // Re-assert the verification meta into the DOM before signing — React
            // hydration may have stripped it since loadAccount. The signer reads
            // querySelectorAll("[name^=gr]")[0].getAttribute("content").
