...
export class GrokClient {
    ...
    setSig(sig) {
        if (!sig || typeof sig !== 'string' || sig.length < 20) {
            console.log(`[Sig] setSig REJECTED — ${!sig ? 'empty/missing' : typeof sig !== 'string' ? 'not a string' : `too short (len=${sig.length}, need >=20)`}`);
            return false;
        }
        this._sig = sig.trim();
        this._sigAt = Date.now();
        console.log(`[Sig] setSig OK — len=${this._sig.length} prefix=${this._sig.slice(0, 6)}… (now fresh, age=0s)`);
        return true;
    }
