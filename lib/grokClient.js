...
export class GrokClient {
    ...
    async chat({ prompt, modeId = 'MODEL_MODE_AUTO', disableSearch = true, onToken, onThink }) {
        ...
        // grok streams NDJSON (one JSON object per line). Parse the same shape
        // the old browser client did.
        const st = { content: '', conversationId: null, lastResponseId: null, error: null, onToken, onThink };
        ...
        while (true) {
            ...
            buf = lines.pop();
            for (const ln of lines) if (ln.trim()) this._onLine(st, ln);
        }
        if (buf.trim()) this._onLine(st, buf);
    ...
    /** Parse one NDJSON line of grok's streaming response. */
    _onLine(st, line) {
