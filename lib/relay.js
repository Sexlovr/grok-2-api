...
export class RelayHub {
    ...
    // ── Worker presence ───────────────────────────────────────────────────
    markWorker() { this.lastWorkerAt = Date.now(); }
    ...
    pushChunk(id, lines, httpStatus) {
        ...
        const job = this.jobs.get(id);
        if (!job) return false;
        if (httpStatus != null) job.httpStatus = httpStatus;
    ...
    sweep() {
        ...
        for (const [id, job] of this.jobs) {
            // Fail fast: queued job never picked up (worker tab frozen/offline).
            // Don't make the client wait out the full 4-min TTL for a silent stall.
            if (job.status === 'queued' && now - job.createdAt > PICKUP_TIMEOUT_MS) {
                job.status = 'error';
                job.error = 'phone worker did not pick up the job within 20s — the grok tab is likely backgrounded/frozen. Bring the grok.com tab to the foreground and retry.';
                this._wakeConsumers(job);
                continue;
            }
