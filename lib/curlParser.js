...
export function parseGrokCurl(curlString) {
    try {
        ...
        // ── x-statsig-id (the anti-bot signature) ──
        // Seeds an initial sig so a freshly-added account can serve for the ~3-4
        // min the sig stays valid, before the refresher userscript takes over.
        let statsigId = null;
        const sig =
            str.match(/-H\s+'x-statsig-id:\s*([^']*)'/i) ||
            str.match(/-H\s+"x-statsig-id:\s*([^"]*)"/i);
        if (sig) statsigId = sig[1].trim();
