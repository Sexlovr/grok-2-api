...
export async function initDB() {
    ...
    // wipe+reseed. In DOM-drive mode the mode_id is the label the worker clicks
    // in grok's "Model select" menu, CONFIRMED from a live screenshot to be
    // exactly: Fast / Auto / Expert / Heavy (no "thinking"/"reasoning" mode
    // exists). Free tier only unlocks Fast; paid (SuperGrok) unlocks the rest —
    // a locked option simply isn't clickable, so the worker keeps Fast.
    const MODEL_VERSION = 4;
    const models = [
        // openai-facing id, display, grok menu label (lowercase)
        ['grok-fast', 'Grok Fast', 'fast'],          // free + paid
        ['grok-auto', 'Grok Auto', 'auto'],          // chooses Fast or Expert
        ['grok-expert', 'Grok Expert', 'expert'],    // paid
        ['grok-heavy', 'Grok Heavy', 'heavy'],       // paid (Team of Experts)
    ];
